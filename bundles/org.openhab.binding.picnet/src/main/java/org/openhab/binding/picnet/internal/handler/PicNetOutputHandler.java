...
/**
 * The {@link PicNetOutputHandler} handles Output modules in PicNet system.
 * Output modules are read-only (control via Virtual addresses).
 *
 * @author Mattia Belloni - Initial contribution
 */
@NonNullByDefault
public class PicNetOutputHandler extends BaseThingHandler {

    private final Logger logger = LoggerFactory.getLogger(PicNetOutputHandler.class);

    private @Nullable PicNetModuleConfiguration config;
    private @Nullable PicNetBridgeHandler bridgeHandler;

    public PicNetOutputHandler(Thing thing) {
        super(thing);
    }
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        // Output modules are read-only, control via Virtual addresses
        logger.debug("Output module {} is read-only, ignoring command. Use Virtual address for control",
                config != null ? config.address : "unknown");
    }

    /**
     * Read the word value from this output module
     *
     * @return the word value or -1 on error
     */
    public int readOutputWord() {
        ...
        if (localConfig == null || localBridgeHandler == null) {
            logger.trace("Cannot read output - configuration or bridge not available");
            return -1;
        }

        SappConnection connection = localBridgeHandler.getConnection();
        if (connection == null || !connection.isConnected()) {
            logger.trace("Cannot read output {} - connection not available", localConfig.address);
            return -1;
        }

        try {
            Sapp75Command command = new Sapp75Command((byte) localConfig.address);
            command.run(connection);

            if (command.isResponseOk()) {
                int value = command.getResponse().getDataAsWord();
                logger.trace("Read output module {}: {}", localConfig.address, value);
                return value;
            } else {
                logger.debug("Failed to read output module {}", localConfig.address);
                return -1;
            }
        } catch (Exception e) {
            logger.debug("Error reading output module {}: {}", localConfig.address, e.getMessage());
            return -1;
        }
    }
